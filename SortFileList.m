function FileListSorted = SortFileList( FileList )
    ...
    RawNames = cellfun(@(x) x(1:end-4),FileList,'UniformOutput',0);
    if isnumeric(str2double(cellstr(num2str((1:length(RawNames))'))))
        Vector = str2double(RawNames(:));
        [~,Indices] = sort(Vector);
        FileListSorted = FileList(Indices);
    else
        
        FileListSorted = cell(size(FileList));
        for Index = 1 : numel(FileList)
            Points                      = strfind(FileList{Index},'.');
            RealIndex                   = str2double(FileList{Index}(Points(4) + 1 : Points(5) - 1));
            FileListSorted{RealIndex}   = FileList{Index};
        end
