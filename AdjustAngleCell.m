function [ GridDataAdj ] = AdjustAngleCell( GridData, CenterX, CenterY, Angle )
    ...
    % angle. In this case the points are stored in a cell

    [SizeY, SizeX]  = size(GridData);
    GridDataAdj     = cell(SizeY, SizeX);
    
    for Index = 1 : numel(GridData)
        if ~isempty(GridData{Index})
            [x,y] = AdjustAngle( GridData{Index}(1), GridData{Index}(2), CenterX, CenterY, Angle );
            GridDataAdj{Index} = [x,y];
        end
     end
end
