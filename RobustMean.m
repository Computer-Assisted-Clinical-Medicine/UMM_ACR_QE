function [ RobMean, RobSTD ] = RobustMean( InVec )
    ...
    %Method Description:
    %-------------------
    %calculate robust mean of input vector

    RawSTD      = std(InVec);
    RawMean     = mean(InVec);
    RobInd      = find(abs(InVec - RawMean) < RawSTD);
    
    RobMean     = mean(InVec(RobInd));
    RobSTD      = std(InVec(RobInd));
