function [ OutputImage ] = MakeBinaryImage( InputImage, Threshold )
    ...
    %Method Description:
    %-------------------
    %Creates a BinaryImage (only 0 and 1 contained as matrix values) using
    %an InputImage and a certain Threshold

    [SizeY, SizeX]  = size(InputImage);
    OutputImage     = nan(SizeY, SizeX);
    
    OutputImage(InputImage < Threshold)     = 0;
    OutputImage(InputImage >= Threshold)    = 1;
