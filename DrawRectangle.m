function DrawRectangle( X1, X2, Y1, Y2, Color )
   ...
   %   Copyright (C) 2013 Heidelberg University 
    ...
    %Method Description:
    %
    %Simple drawing method to plot a rectangle based on four coordinates 
    %with a certain color   

    %Some Error Handling
    if (X1 == X2 || Y1 == Y2)
        Error('WARNING: Coordinate coincidence!')
    else
        line([X1 X2],[Y1 Y1],'Color',Color,'Linewidth',2)
        line([X2 X2],[Y1 Y2],'Color',Color,'Linewidth',2)
        line([X1 X2],[Y2 Y2],'Color',Color,'Linewidth',2)
        line([X1 X1],[Y1 Y2],'Color',Color,'Linewidth',2)
    end
