function Threshold = kmeans(Image)
    ...
    %Method Description:
    %-------------------
    %Perform simple kmeans clustering (k=2)

    IterationDepth  = 10;

    LowClass    = min(min(Image));
    HighClass   = max(max(Image));


    for ItIndex = 1 : IterationDepth
        Threshold = (HighClass + LowClass)/2;
        LowSelection     = Image(Image <  Threshold);
        HighSelection    = Image(Image >= Threshold);

        LowClass         = mean(LowSelection);
        HighClass        = mean(HighSelection);
    end
end