%function [ SpaceX, SpaceY ] = DetermineGridParameter( GridData, CenterX, CenterY )
    ...
    [SizeX, SizeY]  = size( GridData );
    xVec    = nan(numel(GridData),1);
    yVec    = nan(numel(GridData),1);
    ...
    for xInd = 1 : SizeX
        for yInd = 1 : SizeY
            if ~isempty( GridData{ yInd, xInd } )
                ...
                xVal = GridData{ yInd, xInd }(1);
                yVal = GridData{ yInd, xInd }(2);
                xVec( Counter ) = abs(xVal - CenterX)/abs(6 - xInd - 0.5);
                yVec( Counter ) = abs(yVal - CenterY)/abs(6 - yInd - 0.5);
                Counter = Counter + 1;
                
            end
        end
    end
    SpaceX  = mean(xVec(~isnan(xVec)));
    SpaceY  = mean(yVec(~isnan(yVec)));
