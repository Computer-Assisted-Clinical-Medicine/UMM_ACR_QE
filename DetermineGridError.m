function [GridErrorCell, GridErrorVector] = DetermineGridError( GridData, Result_SL, PixelSpacing )
    ...
    %Method Description:
    %
    %From the GridData and the registered ideal grid, calculate the shift
    %of each center to its optimal position
    
    [ SizeY, SizeX ]    = size(GridData);
    Weights             = [-4.5 -3.5 -2.5 -1.5 -0.5 0.5 1.5 2.5 3.5 4.5];
    GridErrorCell       = cell(SizeX, SizeY);
    GridErrorVector     = nan(1,88);
    Counter             = 1;
    ...
    for xInd = 1 : SizeX
        for yInd = 1 : SizeY
            if ~isempty(GridData{yInd, xInd})
                ...
                GridErrorCell{ yInd, xInd }         = [sqrt((IdealXAdj - RealX)^2 + (IdealYAdj - RealY)^2) * PixelSpacing, ...
                                                      atan(-(RealY - IdealYAdj)/(RealX - IdealXAdj))]; 
                if MySign(IdealXAdj - RealX) > 0
                    GridErrorCell{ yInd, xInd }(2) = GridErrorCell{ yInd, xInd }(2) + pi;
                end
                GridErrorVector(Counter)        = sqrt((IdealXAdj - RealX)^2 + (IdealYAdj - RealY)^2) * PixelSpacing;
                Counter                         = Counter + 1;
            end
        end
    end
