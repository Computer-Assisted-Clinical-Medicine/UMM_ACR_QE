function FileName = GetDatasetTick( Dataset )
    ...
    %Method Description:
    %-------------------
    %Extract ticks from the Dataset, only for plotting purpose
    
    Site            = Dataset.Site(1:2);
    Acquisition     = Dataset.Acquisition;
    Inds            = strfind(Acquisition,'_');
    Acquisition(Inds(1))       = ' ';
    Acquisition(Inds(2:end))   = ':';
    
    FileName    = [Site,' ',Acquisition];
