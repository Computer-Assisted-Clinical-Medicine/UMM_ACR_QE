function SaveCommandText( Parameter )
    ...
    %Method Description:
    %-------------------
    %write the text that has been put to the command in a text file and
    %store it to the phantom results folder (only  the result output 
    %is being stored)
    fileID = fopen([Parameter.GEN.SavePath,'/','CommandOutput.txt'],'wt');
    for Index = 1 : numel(Parameter.GEN.CommandCell)
        fprintf(fileID,[Parameter.GEN.CommandCell{Index},'\n']);
    end
